...
$sql = "SELECT id_tamanio, nombre, precio, popular FROM tamanos_impresion";
$stmt = $pdo->prepare($sql);
$stmt->execute();
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $sizes[$row['id_tamanio']] = [
        'name' => $row['nombre'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reset'])) {
        ...
        if (empty($errores)) {
    // 1) Marco el pedido como completado en sesión
    $_SESSION['order']['status']  = 'completed';
    $_SESSION['order']['invoice'] = $invoice = 'INV-' . time();
    $_SESSION['order']['pago']    = [
        'titular'     => $titular,
    ...
    // 2) Inserciones en base de datos
    $usuarioId   = $_SESSION['user_id'];
    $fechaAhora  = date('Y-m-d H:i:s');
    $estadoOrden = 'completada';  // o "pendiente" si lo marcas antes de procesar el pago

    $pdo->beginTransaction();
    try {
        // a) Insertar cada línea de orden
        $sqlOrden = "
            INSERT INTO ordenes_impresion
                (cliente_id, foto_id, cantidad, tamanio_id, precio_unitario, subtotal, estado, fecha_solicitud)
            VALUES
                (:cliente, :foto, :cant, :tam, :pu, :sub, :est, :fecha)
        ";
        ...
        // b) Registrar pago
        $lastOrdenId = $pdo->lastInsertId();  // ID de la última línea insertada
        $sqlPago = "
            INSERT INTO pago_impresion
                (orden_impresion_id, monto, metodo_pago, numero_factura, fecha_pago)
            VALUES
                (:orden, :monto, :metodo, :factura, :fecha)
        ";
        $stmtPago = $pdo->prepare($sqlPago);
        $stmtPago->execute([
            ':orden'   => $lastOrdenId,
            ':monto'   => $_SESSION['order']['total'],
            ':metodo'  => 'tarjeta',
            ':factura' => $invoice,
            ':fecha'   => $fechaAhora,
        ]);
...
document.addEventListener('DOMContentLoaded', () => {
  const form     = document.querySelector('form[name="process_payment_form"]') || document.querySelector('form'); 
  const numero   = document.getElementById('numero-tarjeta');
  const venc     = document.getElementById('vencimiento-tarjeta');
  const cvv      = document.getElementById('cvv');
  const titular  = document.querySelector('input[name="titular"]');

  // 1. Formateo existente
  if (numero) numero.addEventListener('input', () => {
    let d = numero.value.replace(/\D/g, '').slice(0,16);
    numero.value = d.replace(/(.{4})/g,'$1 ').trim();
  });
  if (venc) venc.addEventListener('input', () => {
  ...
  // 4. Custom validators
  function validateField(field, valid, message) {
    if (valid) {
  ...
  if (form) {
    form.addEventListener('submit', e => {
      let ok = true;
      // Titular
      const tval = titular.value.trim();
      const tgood = /^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{5,}$/.test(tval);
      validateField(titular, tgood, 'Nombre inválido');
      if (!tgood) ok = false;

      // Número
      const nval = numero.value;
      const ngood = /^\d{4}( \d{4}){3}$/.test(nval) && luhnCheck(nval);
      validateField(numero, ngood, 'Número de tarjeta inválido');
      if (!ngood) ok = false;

      // Vencimiento
      const vval = venc.value;
      const vgood = validExpiry(vval);
      validateField(venc, vgood, 'Fecha expirada o formato incorrecto');
      if (!vgood) ok = false;

      // CVV
      const cval = cvv.value;
      const cgood = /^\d{3,4}$/.test(cval);
      validateField(cvv, cgood, 'CVV inválido');
      if (!cgood) ok = false;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cvv.addEventListener('input', () => {
    // Elimina todo lo que no sea dígito y recorta a 4 caracteres
    cvv.value = cvv.value.replace(/\D/g, '').slice(0, 4);
  });
});
</script>
